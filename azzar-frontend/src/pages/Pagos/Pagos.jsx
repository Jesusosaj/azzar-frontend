...
function Pagos() {
  ...
  const traerPedidos = async (idCliente) => {
    try {
      const res = await fetch(`https://unlyrical-bronwyn-subangular.ngrok-free.dev/v1/azzar/pedidos/pendientes/${idCliente}`);
  ...
  const eliminarPedido = async (idRifa) => {
    try {
      const res = await fetch(`https://unlyrical-bronwyn-subangular.ngrok-free.dev/v1/azzar/pagos/eliminar/${idRifa}`, {
        method: 'DELETE',
      });
  ...
  const pagar = async () => {
    ...
    try {
      ...
      const response = await fetch("https://unlyrical-bronwyn-subangular.ngrok-free.dev/v1/azzar/pagos/confirmar", {
        method: "POST",
        body: formData,
      });
