...
function IniciarSesion({ onClose }) {
  ...
  const iniciarSesion = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.100.91:8080/v1/azzar/clientes/login", {
        method: "POST",
  ...
  const cambiarPassword = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.100.91:8080/v1/azzar/clientes/cambiar/password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(usuario)
      });
